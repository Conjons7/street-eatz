...
TaskManager.defineTask('watch', ({ data: { locations = [] }, error }) => {
  ...
  const socket = io.connect(`http://192.168.0.156:3000`, { transports: ['websocket'] });
...
export default class Owner extends Component {
  constructor(props) {
    ...
    this.socket = io.connect(`http://192.168.0.156:3000`, { transports: ['websocket'] })
  ...
  componentDidMount() {
    const self = this;
    axios.get(`http://192.168.0.156:3000/api/Owners/${this.props.userId}`)
      .then(res => self.setState({ name: res.data.name }))
    axios.get(`http://192.168.0.156:3000/api/Owners/${this.props.userId}/businesses`)
